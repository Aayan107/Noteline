...
if($count !== 1){
    ...
    if(empty($_POST['rememberme'])){
        ...
        echo "success";
        ?>
        <script> window.location.href = "http://localhost/NOS/mainpageloggedin.php"; </script>
        <?php
        //header("location: http://localhost/NOS/mainpageloggedin.php");
    }else{
        ...
        (`auhenticator1`, `f2authenticator2`, `user_id`, `expires`)
        VALUES
        ('$authenticator1', '$f2authenticator2', '$user_id', '$expiration')";
        $result = mysqli_query($link, $sql);
        echo "$authenticator1   "."$f2authenticator2   "."$user_id   "."$expiration ";
